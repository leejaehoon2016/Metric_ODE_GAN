...
from model.util_change.banach import banach_loss_ODE_split
from model.util_change.gradient_penalty import calc_gradient_penalty_ODE_split
...
class CTGANSynthesizer(BASIC_CTGANSynthesizer):
    ...
    def fit(self, train_data, test_data, meta_data, dataset_name, categorical_columns, ordinal_columns):
        ...
        Mnet = MetricNet_Proxy((data_dim + self.cond_generator.n_opt) * PACK, batch_size, temperature).cuda()
            ...
            for id_ in range(steps_per_epoch):
                for _ in range(D_Learning_iter):
                    ...
                    for _ in range(Metric_Learning_iter):
                        ...
                        Metric_loss = Mnet.train(real_cat, fake_cat, writer, i)
                    ...
                    pen = calc_gradient_penalty_ODE_split(discriminator, real_cat, fake_cat, PACK, self.device, lambda_grad)
                    banach = banach_loss_ODE_split(y_real, y_fake, Mnet, real_cat, fake_cat, writer, i, kind,
